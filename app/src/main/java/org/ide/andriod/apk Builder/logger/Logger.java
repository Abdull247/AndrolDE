...
public class Logger {
	...
	/**
	 * Posts onto the same RecyclerView message queue that d()/e()/w() use,
	 * guaranteeing this runs after every log line already queued at call
	 * time. Use this instead of getLogData() when you need a complete,
	 * ordered snapshot right after a build finishes.
	 */
	public void getLogDataWhenReady(OnLogDataReadyListener listener) {
		if (mRecyclerView == null) {
			listener.onReady(new Gson().toJson(data));
			return;
		}
		mRecyclerView.post(() -> {
			listener.onReady(new Gson().toJson(data));
